...
client.subscribe(commonTopic.CONTROLLER_CURRENT_STATUS, function() {
    console.log('subscribed : ', commonTopic.CONTROLLER_CURRENT_STATUS);
});

// Controller -> Broker 19-07

client.subscribe(commonTopic.CONTROLLER_CREATION, function() {
  console.log('subscribed : ', commonTopic.CONTROLLER_CREATION);
});

client.subscribe(commonTopic.CONTROLLER_STATUS_LEVEL, function() {
  console.log('subscribed : ', commonTopic.CONTROLLER_STATUS_LEVEL);
}); 
...
client.on('message', function(topic, message, packet) {
      ...
      } else if (topic == commonTopic.CONTROLLER_STATUS_LEVEL) {
        controllerReg.controllerStatus(message,client);
      } else if (topic == commonTopic.CONTROLLER_CURRENT_STATUS) {
        controllerReg.controllerCurrentStatus(message,client);
      } 
});
